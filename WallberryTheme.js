		...
		fetch(url, {
			...
			.then(response => {
				if (response.ok) {
					...
					throw new Error(`Unsplash Error: ${response.status}, ${response.statusText}`);
				}
			})
			.then(unsplashData => {
			...
			.catch(error => {
				mod.processError(error.message || "Could not connect to the Unsplash server.");
				Log.error("WallberryTheme Fetch Error: ", error);
			});
	},
